...
/**
 * @file
 * Contains \Drupal\custom_search\Plugin\Block\CustomSearchBlock.
 */
...
class CustomSearchBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $form['search_box']['label'] = array(
      ...
      '#description'    => t('Enter the label text for the search box. The default value is "Search this site".'),
      '#states' => array(
        'visible' => array(
    ...
    $form['search_box']['size'] = array(
      ...
      '#description'    => t('The default value is "!default".', array('!default' => 15)),
    );
    $form['search_box']['max_length'] = array(
      ...
      '#description'    => t('The default value is "!default".', array('!default' => 128)),
      '#required'       => TRUE,
    );
    ...
    if (count($pages_options)) {
      $form['content']['page'] = array(
        '#type'           => 'select',
        '#title'          => t('Search page'),
        '#description'    => t('Select which page to use when searching content with this block. Pages are defined <a href="!link">here</a>.', array('!link' => Url::fromRoute('entity.search_page.collection', array(), array('fragment' => 'edit-search-pages'))->toString())),
    ...
    $form['content']['selector']['label'] = array(
      ...
      '#description'    => t('Enter the label text for the selector. The default value is "Search for".'),
      '#states' => array(
        'visible' => array(
    ...
    if (count($vocabularies)) {
      ...
      foreach ($vocabularies as $voc) {
        ...
        $form['taxonomy'][$vocId]['label'] = array(
          ...
          '#description'    => t('Enter the label text for the selector. The default value is "!default".', array('!default' => $voc->label())),
          '#states' => array(
            'visible' => array(
        ...
        $form['taxonomy'][$vocId]['all_text'] = array(
          ...
          '#required'       => TRUE,
          '#description'    => t('Enter the text for "any term" choice. The default value is "- Any -".'),
        );
      }
    }
    ...
    $form['criteria']['or']['label'] = array(
      ...
      '#description'    => t('Enter the label text for this field. The default value is "Containing any of the words".'),
      '#states' => array(
        'visible' => array(
    ...
    $form['criteria']['phrase']['label'] = array(
      ...
      '#description'    => t('Enter the label text for this field. The default value is "Containing the phrase".'),
      '#states' => array(
        'visible' => array(
    ...
    $form['criteria']['negative']['label'] = array(
      ...
      '#description'    => t('Enter the label text for this field. The default value is "Containing none of the words".'),
      '#states' => array(
        'visible' => array(
    ...
    $form['languages'] = array(
      '#type'         => 'details',
      '#title'        => t('Languages'),
      '#description'  => t("Select the languages to present as search options in the search block. If none is selected, no selector will be displayed. <strong>Note</strong>: if there's only one language checked, the selector won't be displayed BUT only this language will be searched."),
    ...
    $form['languages']['languages'] = array(
      '#type'           => 'checkboxes',
      '#title'          => t('Languages'),
      '#description'    => t('Note: if <em>- Current language -</em> is selected, this current language won\'t be displayed twice.'),
    ...
    $form['languages']['selector']['label'] = array(
      ...
      '#description'    => t('Enter the label text for the selector. The default value is "Languages".'),
      '#states' => array(
        'visible' => array(
    ...
    $form['languages']['any']['text'] = array(
      '#type'           => 'textfield',
      '#title'          => t('- Any language - text'),
      '#default_value'  => $this->configuration['languages']['any']['text'],
      '#required'       => TRUE,
      '#description'    => t('Enter the text for "any language" choice. The default value is "- Any -".'),
    );
    $form['languages']['any']['restricts'] = array(
      '#type'           => 'checkbox',
      '#title'          => t('Choosing - Any - restricts the search to the selected languages.'),
      '#default_value'  => $this->configuration['languages']['any']['restricts'],
      '#description'    => t('If not checked, choosing - Any - will search in all languages.'),
    );
    $form['languages']['any']['force'] = array(
      '#type'           => 'checkbox',
      '#title'          => t('Force - Any - to be displayed.'),
      '#default_value'  => $this->configuration['languages']['any']['force'],
      '#description'    => t('When only one language is selected, the default behaviour is to hide the selector. If you need the - Any - option to be displayed, check this.'),
    );
    ...
    $form['paths']['selector']['label'] = array(
      ...
      '#description'    => t('Enter the label text for the selector. The default value is "Customize your search".'),
      '#states' => array(
        'visible' => array(
    ...
    $form['paths']['separator'] = array(
      ...
      '#description'    => t('Enter a separator that will be used when multiple content types or taxonomy terms are selected and [types] and/or [terms] tokens are used.'),
    ...
    $form['order'] = array(
      ...
      '#description'  => t('Order the form elements as you want them to be displayed. If you put elements in the Popup region, they will only appear when the search field is clicked.'),
      '#open'         => TRUE,
    );
    $form['order']['table'] = array(
      '#type'       => 'table',
      '#header'     => array(t('Element'), t('Region'), t('Weight')),
      '#attributes' => array(
        'id' => 'elements',
      ),
    );
    ...
    $elements = array(
      ...
      'or'          => array(
        'label'   => t('Criteria: Containing any of the words'),
        'config'  => $this->configuration['criteria']['or'],
      ),
      'phrase'      => array(
        'label'   => t('Criteria: Containing the phrase'),
        'config'  => $this->configuration['criteria']['phrase'],
      ),
      'negative'    => array(
        'label'   => t('Criteria: Containing none of the words'),
    ...
    if (count($vocabularies)) {
      foreach ($vocabularies as $voc) {
        ...
        $elements['voc-' . $vocId] = array(
          'label'   => t('Taxonomy: !name', array('!name' => $voc->label())),
    ...
    foreach ($regions as $region => $title) {
      ...
      $form['order']['table'][$region . '-message']['message'] = array(
        '#markup'             => '<em>' . t('No elements in this region') . '</em>',
        '#wrapper_attributes' => array(
          'colspan' => 5,
        ),
      );
      ...
      if (isset($regionsElements[$region])) {
        foreach ($regionsElements[$region] as $id => $element) {
          ...
          $form['order']['table'][$id]['region'] = array(
            '#type'           => 'select',
            '#title'          => t('Region for @title', array('@title' => $element['label'])),
